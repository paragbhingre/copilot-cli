...
func TestNewLoadBalancedWebService_UnmarshalYaml(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			require.Equal(t, tc.wantedStruct.Advanced, rr.PrimaryRoutingRule.HealthCheck.Advanced)
			require.Equal(t, tc.wantedStruct.Advanced.Path, rr.PrimaryRoutingRule.HealthCheck.Advanced.Path)
			require.Equal(t, tc.wantedStruct.Advanced.HealthyThreshold, rr.PrimaryRoutingRule.HealthCheck.Advanced.HealthyThreshold)
			require.Equal(t, tc.wantedStruct.Advanced.UnhealthyThreshold, rr.PrimaryRoutingRule.HealthCheck.Advanced.UnhealthyThreshold)
			require.Equal(t, tc.wantedStruct.Advanced.Interval, rr.PrimaryRoutingRule.HealthCheck.Advanced.Interval)
			require.Equal(t, tc.wantedStruct.Advanced.Timeout, rr.PrimaryRoutingRule.HealthCheck.Advanced.Timeout)
		})
	}
}
...
func TestLoadBalancedWebService_ExposedPorts(t *testing.T) {
	testCases := map[string]struct {
		...
		"expose new sidecar container port through additional rule": {
			mft: &LoadBalancedWebService{
				Workload: Workload{
					Name: aws.String("frontend"),
				},
		...
		"expose new primary container port through alb additional rule": {
			mft: &LoadBalancedWebService{
				Workload: Workload{
					Name: aws.String("frontend"),
				},
		...
		"reference existing sidecar container port through alb additional rule's target_port": {
			mft: &LoadBalancedWebService{
				Workload: Workload{
					Name: aws.String("frontend"),
				},
		...
		"reference existing primary container port through alb additional rule's target_port": {
			mft: &LoadBalancedWebService{
				Workload: Workload{
					Name: aws.String("frontend"),
				},
