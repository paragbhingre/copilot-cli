...
func (r *RoutingRuleConfigOrBool) UnmarshalYAML(value *yaml.Node) error {
	...
	if !r.RoutingRuleConfiguration.MainRoutingRule.IsEmpty() {
		// Unmarshalled successfully to r.RoutingRuleConfiguration, unset r.Enabled, and return.
		r.Enabled = nil
		// this let us treat main listener rule and additional listener rules equally
		// because we eliminate the need of TargetContainerCamelCase by assigning it value to TargetContainer.
...
func (r *ALBRoutingRule) IsEmpty() bool {
	return r.Path == nil && r.ProtocolVersion == nil && r.HealthCheck.IsZero() && r.Stickiness == nil && r.Alias.IsEmpty() &&
		...
		r.HostedZone == nil && r.RedirectToHTTPS == nil && r.TargetPort == nil
}
