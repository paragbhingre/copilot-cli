	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			if tc.wantedError != nil {
				...
				require.Equal(t, tc.wantedStruct.StringSliceOrString, r.PrimaryRoutingRule.Alias.StringSliceOrString)
				require.Equal(t, tc.wantedStruct.AdvancedAliases, r.PrimaryRoutingRule.Alias.AdvancedAliases)
			}
		})
	}
}
