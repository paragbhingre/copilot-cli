...
func (d *lbWebSvcDeployer) validateALBRuntime() error {
	if err := d.validateALBRuntimeFor(d.lbMft.RoutingRule.MainRoutingRule); err != nil {
		return fmt.Errorf("validate ALB runtime configuration for http: %w", err)
	}
	for idx, additionalRule := range d.lbMft.RoutingRule.AdditionalRoutingRules {
...
func (d *lbWebSvcDeployer) validateALBRuntimeFor(rule manifest.ALBRoutingRule) error {
